...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CardGameManagerNetworkObject = null;
		public GameObject[] CardGamePlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CardGameManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CardGameManagerNetworkObject.Length > 0 && CardGameManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CardGameManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CardGameManagerBehavior>();
						}
					}
			...
			else if (obj is CardGamePlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CardGamePlayerNetworkObject.Length > 0 && CardGamePlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CardGamePlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CardGamePlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCardGameManager instead, its shorter and easier to type out ;)")]
		public CardGameManagerBehavior InstantiateCardGameManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CardGameManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<CardGameManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CardGameManagerBehavior>().networkObject = (CardGameManagerNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateCardGamePlayer instead, its shorter and easier to type out ;)")]
		public CardGamePlayerBehavior InstantiateCardGamePlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CardGamePlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<CardGamePlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CardGamePlayerBehavior>().networkObject = (CardGamePlayerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of CardGameManager
		/// </summary>
		/// <returns>
		/// A local instance of CardGameManagerBehavior
		/// </returns>
		/// <param name="index">The index of the CardGameManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public CardGameManagerBehavior InstantiateCardGameManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CardGameManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<CardGameManagerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of CardGamePlayer
		/// </summary>
		/// <returns>
		/// A local instance of CardGamePlayerBehavior
		/// </returns>
		/// <param name="index">The index of the CardGamePlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public CardGamePlayerBehavior InstantiateCardGamePlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CardGamePlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<CardGamePlayerBehavior>();
